...
function Dates() {
  ...
  async function ascending() {
    setLoading(true);
    try {
      const response = await axios.get(" /sort/asc");
      setData(response.data);
      setLoading(false);
    } catch (error) {
      console.log(error);
  ...
  async function descending() {
    setLoading(true);
    try {
      const response = await axios.get(" /sort/dsc");
      setData(response.data);
      setLoading(false);
    } catch (error) {
      console.log(error);
    ...
    <div className="dateParent">
      <div className="dateChild">
        ...
        <button
          ...
          className="btn btn-outline-light rentBtn"
        >
          Rent a Bike
        </button>
        <div className="dropdown">
          ...
          <div className="dropdown-menu" aria-labelledby="dropdownMenu2">
            <button className="dropdown-item" type="button" onClick={ascending}>
              Low To High Price
            </button>
            <button
